...
contract Dfistival is Ownable, ReentrancyGuard, ERC721 {
  ...
  constructor(uint256 _pct) ERC721('Fistival 3 X', 'DFS') Ownable(msg.sender){
    servicePct = _pct;
  }
  ...
  function getEvents() public view returns (EventStruct[] memory Events) {
    ...
    for (uint256 i = 1; i <= _totalEvents; i++) {
      if (!events[i].deleted) {
        available++;
      }
    }
  ...
  // Example of a direct modification
function payout(uint256 eventId) public {
    require(eventExists[eventId], 'Event not found');
    require(!events[eventId].paidOut, 'Event already paid out');
    // Conditionally skip the time check
    if (isTesting) {
        // Skip the time check
    } else {
  ...
  function mintTickets(uint256 eventId) internal returns (bool) {
    for (uint i = 0; i < tickets[eventId].length; i++) {
      ...
      _mint(tickets[eventId][i].owner, _totalTokens);
    }
